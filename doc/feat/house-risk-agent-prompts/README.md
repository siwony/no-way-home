...
이 폴더는 `doc/feat/feature-template.md` 구조에 맞춘 기능 구현 문서입니다. AI agent는 이 폴더의 문서를 읽고 Kotlin + Spring Boot + MariaDB 기준으로 기능을 구현합니다.
...
3. `implementation-notes.md`에 따라 Kotlin 코드, DB, API를 설계한다.
4. `tests.md`의 체크리스트를 테스트로 옮긴다.
5. `constraints.md`를 최종 검수 기준으로 사용한다.
