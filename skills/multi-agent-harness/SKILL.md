...
4. Delegate role work to the matching project TOML subagent in `.codex/agents/` when the user asks for multi-agent or delegated execution.
5. Before handoff, inspect `Status`, `Decision`, and `Result` fields in `doc/agent-work/<work-id>/`.
6. Update only the file for the current agent role unless the user explicitly asks for harness maintenance.
...
- Do not mark work merge-ready unless Director final review is `READY`.
